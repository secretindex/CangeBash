...
import { handleAuthenticateToken } from "./actions";
...
export default function CangeToken() {
  ...
  const authenticateToken = async () => {
    const authToken = await handleAuthenticateToken(token);

    if (!authToken) {
      toast.error("Erro ao autenticar token");
      return;
    }
      ...
      <Card className="w-full bg-card max-w-md">
        <CardHeader>
          ...
          <CardDescription className="text-sm text-muted-foreground">
            Crie um novo Token no Cange e cole aqui. Ele atualiza
            automaticamente.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <CardDescription className="text-sm text-secondary-foreground">
            <div className="rounded-md bg-muted p-2 overflow-hidden">
              ...
              <p className="text-sm text-muted-foreground truncate">
                Criado em:{" "}
                {new Date(tokenContext?.created_at!).toLocaleDateString(
                  "pt-BR",
                  {
                    day: "2-digit",
                    month: "2-digit",
              ...
              <p className="text-sm text-destructive truncate">
                Expira em:{" "}
                {new Date(
                  new Date(tokenContext?.created_at!).getTime() +
                    8 * 60 * 60 * 1000,
